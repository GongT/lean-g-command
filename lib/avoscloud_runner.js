...
var found = module.paths.some(function (p){
	module.exports.runner = path.resolve(p, 'avoscloud-code/bin/run.js');
	return fs.existsSync(module.exports.runner);
});
if(!found){
	console.error();
	process.exit('install avoscloud-code first');
}
...
module.exports.spawn_server = function (){
	var options = {
		...
		env  : extend({
			LC_APP_ID        : config.applicationId,
			LC_APP_KEY       : config.applicationKey,
			LC_APP_MASTER_KEY: config.masterKey,
